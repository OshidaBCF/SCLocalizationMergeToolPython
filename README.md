...
## Why did i do that ?
The short answer is that i was really bored and wanted to use the newly released tool to make a text format i liked when farming for components
At first i was just modifying the strings manually with regex, but then i got curious and wanted to see if there was support for string formatting, as some games supports straight up HTML in text
After more digging i found some interesting stuff in the global.ini, remnants of the common HTML tags, like \<i>, \<b> and even a \<span> with a style that set a background color, but sadly those seems to be extremely old and now deprecated.
I did managed to find 4 tags that allow to change the color of the text that appear when you tractor beam an item, exactly what i wanted (i also found a T pose emote, /wave in chat if you want)
...
This allow me to do THIS, which i think is really cool, even if very limited

<img width="316" height="88" alt="image" src="https://github.com/user-attachments/assets/e783fc78-ed9d-4fbb-991b-2906feb374fc" /><br>

Until i find new way to customize text (if they exist) or until CIG update the game UI system to be something else than Adobe Flash, i don't think i'll be able to add anything
...
And if by a second miracle, a CIG employee is seeing this, please hire me.
